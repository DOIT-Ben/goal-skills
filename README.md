...
`goal-skills` 是一个面向所有 AI 智能体的长期 goal 编译器：它从当前对话、项目现状或产品想法中提取真实意图、项目终极目的、边界和验证标准，生成一段可以直接复制粘贴给 AI 长期执行的 goal 提示词。

包名保留为 `goal-skills`，但公开入口只有一个：`goal-skills`。

## 一句话

把用户刚刚说清楚的目标、约束、偏好和隐含交付物，编译成一份以“现在给你一个目标：xxx”开头、能让 AI 持续执行到完成或停止条件的长期自治提示词。

不同智能体的执行机制不同；这个 skill 负责生成边界清晰、可验证、可迁移的长期 goal 提示词，不把能力边界绑定到某一个平台。

它不只适合已有项目，也适合从一个 idea 开始，把想法变成可以一次推进到可检查结果的 agent goal。

## 什么是 Agent Goal

这里的 goal 是给智能体的长期执行提示词：你给 AI 一个可验证目标，它就能围绕这个目标持续规划、执行、测试、复盘和迭代，直到目标完成、触发停止条件、预算耗尽，或者你手动停止。不同工具的入口不同：在 Codex 里可以作为 `/goal` 使用，在 Claude Code、Cursor、Devin、自建 agent 或其他能读取 Markdown/规则文件的智能体里，可以作为任务 brief 或执行约束使用。

所以 goal 不是普通聊天里的“帮我修一下”，而是更像一份长期作战指令：

- 现在给 AI 什么目标
- 不完成哪些任务不允许停止
- 先如何分析用户意图和项目状态
- 项目的终极目的和成熟状态是什么
- 每轮如何观察、判断、行动、验证、复盘和继续
- 哪些事情不要做，什么时候必须停下来问用户
...
- 它可以改什么、不能改什么
- 它要用什么命令或证据验证
- 它什么时候应该继续，什么时候应该停下来问你

所以 goal 的价值不在于“写得漂亮”，而在于把模糊任务变成可长期执行、可验证、能持续推进项目价值的工作指令。
...
- 先摸清项目，避免一上来乱改
- 把修复、重构、发布、自动化、评估编译成能长期推进的 agent prompt
- 让 AI 先判断项目终极目的，再持续选择下一步
...
然后默认生成一个首选可复制的长期执行 goal prompt。只有目标冲突、风险较高、缺少关键输入，或用户明确要求比较方向时，才先给候选方向让用户选择。
